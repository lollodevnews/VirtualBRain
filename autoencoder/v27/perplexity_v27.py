...
print("==========================================")
print(" VIRTUALBRAIN V27 PERPLEXITY BENCHMARK    ")
print("==========================================")

# --- CONFIGURATION ---
MODEL_ID = os.path.expanduser("~/models/quant/qwen25_7b")
VBR_DIR = os.path.expanduser("~/models/quant/vbr_qwen25_v25")
DEVICE = "cuda:0"
SEQ_LENGTH = 2048
STRIDE = 512

def decompress_v27_matrix(vbr_dict, device="cpu"):
    """Reverses the V27 Autoencoder Bit-Packing purely on the CPU"""
    ...
    powers_of_2 = torch.tensor([1, 2, 4, 8, 16, 32, 64, 128], dtype=torch.int32, device=device)
    
    for i in range(out_features):
        D = int(headers_D[i].item())
        mag_bits = D - 1
        bytes_per_row = mag_bits + 1 # +1 for the sign byte
        
        offset = offsets[i].item()
        
        # Unpack exactly as V27 packed it (flattened row bytes)
        row_groups = vbr_data[offset : offset + (num_groups * bytes_per_row)].view(num_groups, bytes_per_row)
        
        mag_ints_groups = torch.zeros((num_groups, 8), dtype=torch.int32, device=device)
        for bit_idx in range(mag_bits):
            byte_col = row_groups[:, bit_idx].unsqueeze(1)
            bits = (byte_col & powers_of_2) > 0
            mag_ints_groups += bits.int() << bit_idx
            
        sign_byte = row_groups[:, -1].unsqueeze(1)
        signs = (sign_byte & powers_of_2) > 0
        
        mag_ints_flat = mag_ints_groups.flatten()
        signs_flat = signs.flatten()
        
        # Dequantize using the polynomial
        max_int = (2 ** mag_bits) - 1.0
        norm_w = mag_ints_flat.float() / max_int
        ...
        weights = mags * torch.where(signs_flat, -1.0, 1.0)
...
def main():
    ...
    print("\n[*] Moving model to CPU to safely inject V27 compressed matrices...")
    model.to("cpu")
    ...
    for pt_file in pt_files:
        ...
        chunk_data = torch.load(pt_file, map_location="cpu")
        
        for name, vbr_dict in tqdm(chunk_data.items(), desc="Injecting", leave=False):
            if "is_vbr_compressed" not in vbr_dict:
                continue 
                
            submodule = model.get_submodule(name)
            
            if vbr_dict["is_vbr_compressed"]:
                recon_weight = decompress_v27_matrix(vbr_dict, device="cpu")
    ...
    print("\n[*] Moving V27 Compressed model back to GPU...")
    model.to(DEVICE)
    
    print("\n--- MEASURING V27 COMPRESSED ---")
    v27_ppl = calculate_perplexity(model, encodings)
    
    degradation = v27_ppl - base_ppl
    print("\n==========================================")
    print(f" BASE FP16 PERPLEXITY: {base_ppl:.4f}")
    print(f" V27 SWARM PERPLEXITY: {v27_ppl:.4f}")
